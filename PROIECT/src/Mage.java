...
public class Mage extends Character{
    ...
    public Mage(List<Spell> ablities, int currentHealth, int maxHealth, int currentMana, int maxMana, boolean fire_protection, boolean ice_protection, boolean earth_protection, String name, int linie, int coloana, Inventory potionInventory, int experience, int level, int strength, int charisma, int dexterity, int greutateInventar) {
        super(ablities, currentHealth, maxHealth, currentMana, maxMana, fire_protection, ice_protection, earth_protection, name, linie, coloana, potionInventory, experience, level, strength, charisma, dexterity, greutateInventar);
    }
    ...
    public void receiveDamage(int damage) {
        ...
        if(this.getDexterity() > 10 && this.getStrength() > 10) {
    ...
    public int getDamage() {
        ...
        Random random = new Random();
        if(this.getCharisma() >= 20) {
            int nr = random.nextInt(100);
            if(nr <= 50) {
                System.out.println("L-ati lovit pe inamic cu damage dublu!");
                return 40;
            }
            else
                return 20;
